...
- 使用函数，可以让代码在可理解性、可维护性上 有很大的提升 正确

    
- 函数体代码的语句缩进必须是4个空格 正确

    
- 下面的代码，解释器执行完函数func1的代码后，还会返回接着执行func1() 下面的代码 print 'after call'  正确
...
- 函数的定义 和函数的调用在代码文件中 前后次序并不重要，只要都存在即可 错误

    
- 函数的返回值可以是多个对象，中间用逗号隔开 正确
    
    
- 获取 34,67,22,11 ,88 这些数字中最大的数字， 下面的python代码都是正确的 正确
...
- 对一个字符串 a，我们用正数索引访问 最后一个元素，可以这样写 a[len(a)]  错误

    
- 内置函数type，可以查看所有python对象的类型，包括函数对象 正确

    
- 下面的代码执行完后，变量b 的值仍然为 'a'  正确
def t2(para):
    para = 3
...
- 对象方法其实就是隶属于该对象的函数 正确
    
- 代码 'my name 123 jack'.endswith('jack') 返回结果是 True 正确
    
- 代码 'my name 123 jack'.endswith('ack') 返回结果是 True 正确
        
- 代码 'my name 123 jack'.startswith('ack') 返回结果是 True 错误
    
- 代码 '1111'.isalpha() 返回结果是 True 错误
    
- 代码 '111a'.isdigit() 返回结果是 True 错误
    
- 代码 'my name is jack'.split(' ')[-1]   的结果是 'jack'  正确
    
- 字符串的replace方法只能替换第一个找到的子字符串,比如
'hi, jack, you are smart, jack'.replace('jack','mary')  错误

    

    
- 列表的append方法只能添加元素到列表尾部， 而insert方法只能插入元素到列表的头部  错误

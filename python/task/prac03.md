...
- 一个变量定义如下
    a = ['this', 'test', 4.56, ['inner', 'list']]
    那么 修改其中的最后一个List元素的第二个元素 值为 8 ，下面的写法正确的是 C,D
...
- 定义一个变量var1 其值是一个字符串 what's your name， 下面哪些是错误的 B
...
- 由于list 是可以改变元素的，当我们对list 进行切片（slice）操作的时候，原来的list 对象就改变了。 比如 执行下面代码后 错
a = [1,2,3,4]
a[1:3]
...
str1 = 'name: Jack, weight: 130kg' 错

要用切片的方式从str1中取出人名  可以是  str1[7:11]   


- 长度为n的字符串str1，最后一个元素 的代码表示可以是str1[-1]也可以是str1[n] 错

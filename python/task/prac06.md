...
- python3.6 执行下面格式化字符串的代码，哪些是会报错的 B
...
- 格式化字符串 '%10d' 表示参数显示的宽度至少为10个字符，不足10个字符用空格补齐  正确

- 格式化字符串 '%010d' 表示参数显示的宽度至少为10个字符，不足10个字符用字符0补齐 正确
    
- 格式化字符串 "%06X" 表示参数以8进制显示，显示的宽度至少为10个字符，不足10个字符用字符0补齐 错误

- 格式化字符串'%9.2f'表示参数数字的小数部分保留2位， 同时还指明了最小宽度是9，不足补空格 正确

- '{:<10}'.format(8) 表示最小宽度是10，同时数字显示在左边，不足10位右边补空格 正确


- 下面代码中变量var1 的长度为4个字符  错误
  var1 =  'a\nb' 
  
  
- 代码的注释，不影响代码的执行，项目开发中我们不写没关系  错误
